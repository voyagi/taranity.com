 ...
 * and everything here animates them in. Gated on [data-storefront] so it does
 * nothing on other designs' pages after a View-Transition swap, and torn down
 * (Lenis included) before every swap so designs never double-drive the scroll.
 *
...
function teardown() {
  ...
  document.querySelector<HTMLElement>('[data-f-progress]')?.style.removeProperty('transform');
}

function setup() {
  teardown();
  const root = document.querySelector<HTMLElement>('[data-storefront]');
  // Reduced motion: storefront.css never hides anything and there is no scroll
  ...
  const progress = document.querySelector<HTMLElement>('[data-f-progress]');
