...
| Hosting | **Cloudflare Pages** | Free unlimited bandwidth, global sub-50ms edge, first-class static Astro support. Static needs **no adapter** - Astro prerenders straight to `dist/` (written for v6, equally true on v7; the v6-era Cloudflare-adapter+static deploy bug is avoided by staying adapter-free). |
